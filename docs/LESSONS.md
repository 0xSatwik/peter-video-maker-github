...
- tmpfiles.org upload of an 82 MB mp4 failed (and its API only guarantees ~100 MB) -> use sto.care (`PUT https://ul.sto.care/<name>`, 100 MB, 72 h, direct link) with gofile as the >100 MB fallback. Zipping an MP4 saves nothing.
